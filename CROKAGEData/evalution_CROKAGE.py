...
def read_CROKAGE_data_from_excel(path):
    ...
    for i in range(nrows):
        ...
        if i>0:#第一行为表头，数据从第二行开始（i+1）
            ...
            CROKAGE_All_Results_Rank = sheet.cell_value(i,5)
            print(query)
            print(str(CROKAGE_All_Results_Rank).split(','))
            rank_list=[]
            for rank in str(CROKAGE_All_Results_Rank).split(','):
    ...
    # print(nrows,ncols)
    print(CROKAGE_All_Results_Rank_list)
...
if __name__ == '__main__':
    path=r'D:/StackOverflow1/DataEvalation/CROKAGE_Data/CROKAGE_UserStudyData_24_GoogleSearched.xls'#413个results的MAP、MRR：3.305 0.915
    CROKAGE_data_list=read_CROKAGE_data_from_excel(path)
    compute_MRR_MAP(CROKAGE_data_list)
    #24个results的MAP、MRR、FR、Top@k：2.379 0.645 2.625 0.958
    # Top-10个结果中能够返回查询的正确结果与返回失败的query个数分别为：23 1

    Google_data_list = read_Google_data_from_excel(path)
    compute_MRR_MAP(Google_data_list)
    # 24个results的MAP、MRR、FR、Top@k：5.337 0.979 1.042 1.000
    # Top-10个结果中能够返回查询的正确结果与返回失败的query个数分别为：24 0

    Bing_data_list = read_Bing_data_from_excel(path)
    compute_MRR_MAP(Bing_data_list)
    #24个results的MAP、MRR、FR、Top@k：3.273 0.896 1.208 1.000
    #Top-10个结果中能够返回查询的正确结果与返回失败的query个数分别为：24 0