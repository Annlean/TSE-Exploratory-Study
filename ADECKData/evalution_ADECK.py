...
def read_ADECK_data_from_excel(path):
    ...
    for i in range(nrows):
        ...
        if i>0:#第一行为表头，数据从第二行开始（i+1）
            ...
            ADECK_All_Results_Rank = sheet.cell_value(i,4)
            print(query)
            print(str(ADECK_All_Results_Rank).split(','))
            rank_list=[]
            for rank in str(ADECK_All_Results_Rank).split(','):
    ...
    # print(nrows,ncols)
    print(ADECK_All_Results_Rank_list)
...
if __name__ == '__main__':
    path=r'D:/StackOverflow1/DataEvalation/ADECKData/ADECKData.xls'# 20个results的MAP、MRR：3.305 0.915
    ADECK_data_list=read_ADECK_data_from_excel(path)
    compute_MRR_MAP(ADECK_data_list)
    #20个results的MAP、MRR、FR、Top@k：4.037 0.664 3.950 0.750
    #Top-10个结果中能够返回查询的正确结果与返回失败的query个数分别为：15 5

    Google_data_list = read_Google_data_from_excel(path)
    compute_MRR_MAP(Google_data_list)
    #20个results的MAP、MRR、FR、Top@k：5.179 0.798 1.700 1.000
    #Top-10个结果中能够返回查询的正确结果与返回失败的query个数分别为：20 0

    Bing_data_list = read_Bing_data_from_excel(path)
    compute_MRR_MAP(Bing_data_list)
    #20个results的MAP、MRR、FR、Top@k：4.095 0.746 1.950 0.950
    #Top-10个结果中能够返回查询的正确结果与返回失败的query个数分别为：19 1