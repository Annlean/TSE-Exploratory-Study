...
def read_KG2CodeData_data_from_excel(path):
    KG2Code_All_Results_Rank_list=[]
    ...
    for i in range(nrows):
        ...
        if i>0:#第一行为表头，数据从第二行开始（i+1）
            ...
            KG2Code_All_Results_Rank = sheet.cell_value(i,4)
            print(query)
            print(str(KG2Code_All_Results_Rank).split(','))
            rank_list=[]
            for rank in str(KG2Code_All_Results_Rank).split(','):
                rank_list.append(int(float(rank)))
            temp_list=[]
            temp_list.append(rank_list)
            temp_list.append(len(ground_truth.split(';')))
            KG2Code_All_Results_Rank_list.append(tuple(temp_list))
    # print(nrows,ncols)
    print(KG2Code_All_Results_Rank_list)

    return KG2Code_All_Results_Rank_list
...
if __name__ == '__main__':
    path = r'D:/StackOverflow1/DataEvalation/KG2CodeData/KG2CodeDataResults.xls'  #53 queries的MAP、MRR：3.305 0.915
    KG2CodeData_data_list=read_KG2CodeData_data_from_excel(path)
    compute_MRR_MAP(KG2CodeData_data_list)
    #53个results的MAP、MRR、FR、Top@k：4.557 0.973 1.208 0.981
    #Top-10个结果中能够返回查询的正确结果与返回失败的query个数分别为：52 1

    Google_data_list = read_Google_data_from_excel(path)
    compute_MRR_MAP(Google_data_list)
    #MAP、MRR、FR、Top@k：4.380 0.991 1.019 1.000
    #Top-10个结果中能够返回查询的正确结果与返回失败的query个数分别为：53 0

    Bing_data_list = read_Bing_data_from_excel(path)
    compute_MRR_MAP(Bing_data_list)
    #53个results的MAP、MRR、FR、Top@k：4.224 0.929 1.321 0.981
    #Top-10个结果中能够返回查询的正确结果与返回失败的query个数分别为：52 1
